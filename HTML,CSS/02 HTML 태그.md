...
+ **내부링크**
  ...
  + 보통 페이지에서 내용이 많아 스크롤이 길어질 경우 빠르게 화면 최상단으로 이동하고자 할때 내부링키를 주로 사용하는데, 웹페이지에서 화면 하단에 있는 'top' 또는 '맨 위로 이동하기' 버튼이 이에 해당한다.
...
+ alt :  이미지의 대체 텍스트를 입력한다. 
  ...
  + 일반적인 상황에서는 노출이 되지 않고, 웹 접근성 측며에서 상당히 중요한 속성이다.
