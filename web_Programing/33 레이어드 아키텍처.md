...
+ 예를 들어 "출금"이라는 기능의 흐름이 다음과 같다고 새각해보자.
  1. 잔액이 얼마인지 조횐한다.
...
+ 방멱록이 없으면 건수는 0이 나오고 아래에 방명록을 입력하는 폼이 보여진다.
...
+ ~~~java
  ...
  @Configuration
  @ComponentScan(basePackages= {"kr.or.connet.guestbook.dao","kr.or.connect.guestbook.service"})
  ...
  + 여기서는 dao나 servie에 구현되어 있는 컴포넌트들을 읽기 때문에 basePackages에다가 각각의 패키지를 지정하고 있는 것을 볼 수 있다.
...
+ ~~~xml
  ...
  + ~~~xml
    ...
    + Context가 로딩되는 이벤트가 일어났을 때 ContextLoaderListener 클래스를 실행시켜줘요라는 의미임
