...
public void merge(int low, int mid, int high){
  ...
  // 나눈 리스트의 대소 비교와 정렬
  // 아래 와일문은 밑의 합병 정렬 참고자료의 움짤을 확인하면 왜 i가 mid까지 ++되고, j가 high까지 ++되는지 이해할 수 있음
  while( i<=mid && j<= high){
    if(array[i]<=array[j]) // 리스트의 i번째 요소가 j번째 요소보다 작은경우
      temp[tempposn++] = array[i++];
    else // 리스트의 j번째 요소가 i번째 요소보다 작은 경우
      temp[tempposn++]= array[j++];
  }
  // i가 mid로 가고 j가 high로 갈 때까지 반복
  // 위 코드에서 대소 비교가 끝나고, 남은 요소들을 정렬해준다.
  // 즉 만약 첫번째 리스트의 모든 요소가 두번째 리스트의 첫번째 요소보다 작아서 
  // i는 mid에 도달했지만 j는 high에 도달하지 못해서 비교하지 못한 요소들이 있다면
  // 아래 코드를 통해 정렬되어 해결된다.
  // 그래서 아래 두 개의 while문 중 하나만 실행된다.
  // 두 개의 while문이 모두 실행되는 경우는 없다. 
  // 두개의 while문이 실행 됐다는 의미는 i, j 모두 mid나 high에 도달하지 못했다는 의미인데
 	// 그런 경우에는 위에 있는 코드에서 대소 비교를 하게 된다.
  while(i <= mid)
    temp[tempposn++] = array[i++];
  while(j <= high)
    temp[tempposn++] = array[j++];
...
+ 퀵 정렬(Quicksort)은 **중심점(Pivot Point)을 임의로 고른 후 이 중심점보다 작은 수를 한 쪽으로 분류하고 큰 수들은 다른 쪽으로 분류하여 정렬하는 방법** 이다.

+ 퀵 정렬 알고리즘은 Python, C, C++의 표준 라이브러리나 Java의 sort 메소드에서 사용하는 정렬 알고리즘이다. 그리고 모든 종류의 변수에 대해 사용 가능하다.

+ 중심점의 왼쪽에 있는 숫자들은 오른쪽에 있는 숫자들과 비교할 필요가 없다.

  + 리스트를 분리하여 따로 비교하므로, **평균적인 경우**에 **시간 복잡도는 O( n log n )** 이다.

    

![mceclip0 (7)](https://user-images.githubusercontent.com/88477839/175802007-fbf82514-c516-4494-a861-745fbdf7fcf2.png)

+ 퀵 정렬은 다음과 같은 순서로 이뤄진다.
  1. 중심점을 선택한다. 
     + 주로 리스트의 중간에 있는 숫자 혹은 마지막에 있는 숫자를 선택한다.
  2. 마지막 요소와 중심점의 위치를 바꿔 중심점을 리스트의 가장 끝으로 보낸다.
  3. 2개의 카운터를 사용하여 리스트의 처음부터 탐색한다. 첫 번째 카운터에는 중심점보다 큰 숫자의 위치를 저장하고 두 번째 카운터에는 현재 탐색하고 있는 위치를 저장한다.
  4. 탐색하며 중심점보다 작은 숫자를 발견하면 첫 번째 카운터의 숫자 위치와 바꿔준다.
  5. 3, 4 과정을 반복하면 중심점보다 큰 수와 중심점보다 작은 수가 나눠지게 된다.
...
> 퀵 정렬에서 최악의 경우가 발생했을 때의 시간 복잡도에 대해 살펴보자



### 퀵 정렬(최악의 케이스)

![mceclip0 (8)](https://user-images.githubusercontent.com/88477839/175802400-83b1026f-5178-495a-99d8-fb7e0dd7089c.png)



+ 퀵 정렬에서는 전체 데이터를 계속해서 반으로 줄여나가기 때문에 **평균적인 상황**에서는 **O( n log n\)**의 시간 복잡도를 갖는다. 
+ 하지만 **최악의 경우**, **계속해서 최소인 지점을 중심점으로** 잡는다면 시간 복잡도는 **O(** n^2 **)**까지도 늘어날 수 있다. (모든 숫자가 중심점보다 커서 대소비교만 하고 분류가 되지 않는 경우이다.)
...
public class QuickSort<E>{
  ...
  public void quickSort(int from, int to){
    if(from >= to) //리스트에 요소가 하나밖에 없다면 정렬 종료
      return;
    //중심점(Pivot Point)으로 배열의 마지막에 있는 숫자를 선택한다.
    E value = array[to];
    // 중심점보다 큰 값을 가리키는 카운터(인덱스)
    int counter = from;
    //중심점의 바로 앞까지 탐색
    for(int i = from; i<to; i++){
      ...
      if(((Comparable<E>)array[i]).compareTo(value)<=0){
        swap(i, counter);
        counter++; // 스왑을 하고 현재 카운터가 더 이상 큰 값을 가리키지 않기 때문에 ++해주어 큰 값을 가르키게 한다.
      }
    }
    //중심점이 중간에 오게 한다.
    swap(counter,to);
    //배열의 왼쪽과 오른쪽을 정렬한다.
    quickSort(form,counter-1);
...
+ 기수 정렬(Radix)은 **우편번호, 자릿수 등의 기준을 만들어 기준에 따라 숫자를 분류하여 정렬하는 방법** 이다.

+ 예를 들어, 4자리 숫자를 일의 자리의 숫자, 십의 자리의 숫자, 백의 자리의 숫자, 천의 자리의 숫자에 따라 분류하여 정렬할 수 있다. 이럴 경우 시간 복잡도는 4n이 된다.

  

+ 이론적으로 이 방법은  n **의 복잡도**를 갖는 빠른 정렬 방법이다.
