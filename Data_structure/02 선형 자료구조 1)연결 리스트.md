...
public E removeLast(){
  ...
  while(current!=tail){ // current == tial 될때까지 루프 돌림
    previous = current;
    current = current.next; // -> 순서 중요 만약 이 두 줄 코드 순서가 바뀌면 결국 마지막 노드를 가리키는 포인터가 previous와 current 둘다가 됨
  }
  ...
  while(current !=null){
    if(((Comparable<E>)current.data.compareTo(obj)==0){
       ...
       if(current==tail) // 마지막 노드 제거
         return removeLast();
       currentSize--;
       previous.next=current.next; // current 포인터가 가르키도 있는 노드를 가비지 컬렉션 만듦. 
       return current.data;
    }
    previous = current;
...
+ ListI 인터페이스를 구현한 LinkedList를 테스트하는 방법은 다음과 같습니다.
...
~~~java
public class LinkedList<E> implements ListI<E>{ //ListI가 아마도 Iterable을 상속하는듯함.
